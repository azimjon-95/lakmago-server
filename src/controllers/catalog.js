...
export const restaurantController = {
  ...
  list: asyncHandler(async (req, res) => {
    ...
    const restaurants = await Restaurant.find(filter)
      .select('name cuisine category kind rating reviewCount deliveryMin deliveryMax deliveryFee freeDeliveryThreshold minOrderAmount discount isFresh tint icon images imageUrl createdAt pickupEnabled prepMinutes shopTypes')
      .sort({ createdAt: -1 })
      .limit(limit + 1)
      .lean();
